module InPlaceMacrosHelper
  ...
  def in_place_editor(field_id, options = {})
    ...
    js_options['loadTextURL'] = "'#{url_for(options[:load_text_url]).gsub('&amp;', '&')}'" if options[:load_text_url]        
    js_options['ajaxOptions'] = options[:options] if options[:options]
    js_options['htmlResponse'] = !options[:script] if options[:script]
    js_options['evalScripts'] = options[:script] if options[:script]
    js_options['callback']   = "function(form) { return #{options[:with]} }" if options[:with]
    js_options['clickToEditText'] = %('#{options[:click_to_edit_text]}') if options[:click_to_edit_text]
    js_options['cancelText'] = "'#{options[:cancelText]}'" if options[:cancelText]#added
    js_options['onComplete'] = options[:onComplete] if options[:onComplete] #added
